...
async function startR(){
    ...
    xcaa.sendMessage(anu.id, {audio: butff, ptt: true, contextInfo: { mentionedJid: [num], mimetype: "audio/mp4", externalAdReply: {showAdAttribution: true, containsAutoReply: true, title: teks1, body: `${ddate}`, previewType: "PHOTO", thumbnailUrl: bufft, thumbnail: bufft, sourceUrl: global.linkgc}}})
    } else if (anu.action == 'remove') {
    let lvpantex = fs.readFileSync('./media/leavee.mp3')
    let lvbuft = await getBuffer(ppuser)
    teks2 = `Sayonara @${num.split("@")[0]} 👋`
    xcaa.sendMessage(anu.id, {audio: lvpantex, ptt: true, contextInfo: { mentionedJid: [num], mimetype: "audio/mp4", externalAdReply: {showAdAttribution: true, containsAutoReply: true, title: teks2, body: `Pantek`, previewType: "PHOTO", thumbnailUrl: lvbuft, thumbnail: lvbuft, sourceUrl: global.linkgc}}})
    }
    }
    } catch (err) {
      console.log(err)
