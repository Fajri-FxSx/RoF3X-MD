...
async function startR(){
    ...
    try {
       ...
       require("./xcaa.js")(xcaa, m, chatUpdate, store)
      } catch (err) {
       console.log(err)
     }
    })
    ...
    xcaa.sendMessage(anu.id, {audio: global.weel, mimetype: "audio/mp4", ptt: true, duration: 9999900000, contextInfo:{mentionedJid: [num], "externalAdReply":{"title": teks1,"body": `To ${metadata.subject}`, "mediaType": 1, "thumbnailUrl": ppuser, "thumbnail": bufft,"sourceUrl": global.linkgc, containsAutoReply: true, showAdAttribution: true, renderLargerThumbnail: true}}})
    } else if (anu.action == 'remove') {
    let lvbuft = await getBuffer(ppuser)
    teks2 = `Sayonara @${num.split("@")[0]} 👋`
    xcaa.sendMessage(anu.id, {audio: global.beey, ptt: true, duration: 9999900000, contextInfo: { mentionedJid: [num], mimetype: "audio/mp4", externalAdReply: {showAdAttribution: true, containsAutoReply: true, title: teks2, body: `Pantek`, previewType: "PHOTO", thumbnailUrl: lvbuft, thumbnail: lvbuft, sourceUrl: global.linkgc}}})
    } else if (anu.action == 'promote') {
    xcaa.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Promote From ${metadata.subject}` })
    } else if (anu.action == 'demote') {
    xcaa.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Demote From ${metadata.subject}` })
    }
    }
    } catch (err) {
      console.log(err)
    ...
    xcaa.getName = (jid, withoutContact  = false) => {
       ...
       resolve(v.name || v.subject || v.lc || v.lg || v.verifiedName || PhoneNumber('+' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
       })
       else v = id === '6283815956151@s.whatsapp.net' ? {
       id,
       name: 'FxSx'
       } : id === xcaa.decodeJid(xcaa.user.id) ?
       xcaa.user :
       (store.contacts[id] || {})
       return (withoutContact ? '' : v.name) || v.subject || v.lc || v.lg || v.verifiedName || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international')
    }
     ...
     ginih = await xcaa.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
     xcaa.sendMessage(jid, {text: 'Nih Kak Owner Ku'}, {quoted: ginih})
    }
    ...
    xcaa.public = true // di true in auto public kalo di false auto self
    ...
    return xcaa.sendMessage(jid, { document: await getBuffer(url), mimetype: mime, caption: caption, ...options }, { quoted: quoted, ...options })
    }
    }
