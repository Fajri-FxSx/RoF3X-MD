...
async function startR(){
    ...
    xcaa.sendMessage(anu.id, {audio: global.weel, ptt: true, contextInfo: { mentionedJid: [num], mimetype: "audio/mp4", externalAdReply: {showAdAttribution: true, containsAutoReply: true, title: teks1, body: `${ddate}`, previewType: "PHOTO", thumbnailUrl: bufft, thumbnail: bufft, sourceUrl: global.linkgc}}})
    } else if (anu.action == 'remove') {
    let lvbuft = await getBuffer(ppuser)
    teks2 = `Sayonara @${num.split("@")[0]} 👋`
    xcaa.sendMessage(anu.id, {audio: global.beey, ptt: true, contextInfo: { mentionedJid: [num], mimetype: "audio/mp4", externalAdReply: {showAdAttribution: true, containsAutoReply: true, title: teks2, body: `Pantek`, previewType: "PHOTO", thumbnailUrl: lvbuft, thumbnail: lvbuft, sourceUrl: global.linkgc}}})
    }
    }
    } catch (err) {
      console.log(err)
    ...
    xcaa.getName = (jid, withoutContact  = false) => {
       ...
       else v = id === '0@s.whatsapp.net', '6283815956151@s.whatsapp.net' ? {
       id,
       name: 'WhatsApp'
