...
module.exports = xcaa = async(xcaa, m, chatUpdate, store) => {
  ...
  const reply2 = (teks) => {
  xcaa.relayMessage(m.chat, {requestPaymentMessage: {noteMessage: {extendedTextMessage: {text: teks, currencyCodeIso4217: 'USD', requestFrom: `${m.sender.split('@')[0]}@s.whatsapp.net`, expiryTimestamp: 8600, amount: 10000, background: thumb }}}})
  }
  
  const reply3 = (hehe) => {
  xcaa.sendMessage(hehe, {audio: global.vnerror, contextInfo: {mentionedJid: [m.sender], externalAdReply: {showAdAttribution: true, title: `Hai ${pushname}`, mediaType: 1, body: `${wib}`, thumbnail: thumb2, previewType: "IMAGE", sourceUrl: global.linkgc}}}, { quoted: m })
  }
...
`
  let btnn = [
     {urlButton: {displayText: 'Website', url: global.roflix}},
...
`
  ...
  buttons: buttons,
  headerType: 4,
  contextInfo:{externalAdReply: {showAdAttribution: true, containsAutoReply: true, title: `Hai ${pushname}`, body: `${wib}`, previewType: "IMAGE", thumbnailUrl: ``, thumbnail: global.thumb2, sourceUrl: global.roflix2}}
  }
  xcaa.sendMessage(m.chat, buttonMessage, { quoted: ftroli })
  } catch (err) {
  reply3(m.chat)
  ...
  const template = generateWAMessageFromContent(yoi, proto.Message.fromObject({ 
   templateMessage: { 
    hydratedTemplate: { 
     ...
     hydratedFooterText: global.author, 
     hydratedButtons: [
     {urlButton: {displayText: 'RoFlix Film', url: 'https://roflix.vercel.app/'}},
     {callButton: {displayText: 'Number Owner', phoneNumber: global.ownerNumber2}},
     {quickReplyButton: {displayText: 'Status Bot', id: 'ping'}},
     {quickReplyButton: {displayText: 'Owner Bot', id: 'owner'}}
     ]
...
case 'join': {
  ...
  await xcaa.groupAcceptInvite(result).then((res) => m.reply(mess.sukses)).catch((err) => reply3(m.chat))
  } catch (err) {
  reply3(m.chat)
  }
  }
...
case 'block': {
  ...
  await xcaa.updateBlockStatus(users, 'block').then((res) => m.reply(mess.sukses)).catch((err) => reply3(m.chat))
  } catch (err) {
  reply3(m.chat)
  }
  }
...
case 'unblock': {
  ...
  try {
  let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
  await xcaa.updateBlockStatus(users, 'unblock').then((res) => m.reply(mess.sukses)).catch((err) => reply3(m.chat))
  } catch (err) {
  reply3(m.chat)
  }
  }
...
case 'igdl': case 'instagram': case 'ig': {
  if (!text) return m.reply(`*Contoh : ${prefix + command} Linknya*`)
  if (!isUrl(args[0])) return reply3(m.chat)
  if (!text.includes('instagram.com')) return reply3(m.chat)
  m.reply(mess.wait)
...
case 'tiktok': case 'tiktoknowm': case 'ttnowm': case 'tiktokvideo': case 'ttvideo': {
  if (!text) return m.reply(`*Contoh : ${prefix + command} Linknya\nJika Tidak Kedetect, Silahkan Berikan Spasinya 2×*`)
  m.reply(mess.wait)
  if (!text.includes('tiktok')) return m.reply('*Pastikan Link Sudah Benar*')
