...
`
  ...
  buttons: buttons,
  headerType: 4,
  contextInfo:{externalAdReply: {showAdAttribution: true, renderLargerThumbnail: true, containsAutoReply: true, title: `Hai ${pushname}`, body: `${wib}`, mediaType: "PHOTO", mediaUrl: global.thumb2, thumbnail: global.thumb2, sourceUrl: global.roflix2}}
  }
  xcaa.sendMessage(m.chat, buttonMessage, { quoted: ftroli })
  } catch (err) {
  m.reply(mess.error)
...
case 'owner': {
  try {
  xcaa.sendContact(m.chat, global.ownerNumber, m, contextInfo:{externalAdReply: {showAdAttribution: true, containsAutoReply: true}})
  } catch (err) {
  m.reply(mess.error)
  }
  }
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
  `.trim()
  xcaa.sendMessage(m.chat, {text: respon, contextInfo:{externalAdReply: {showAdAttribution: true, containsAutoReply: true}}, {quoted: m})
  } catch (err) {
  m.reply(mess.error)
  }
  }
