...
module.exports = xcaa = async(xcaa, m, chatUpdate, store) => {
  ...
  const reply2 = (teks) => {
  xcaa.relayMessage(m.chat, {requestPaymentMessage: {noteMessage: {extendedTextMessage: {text: teks, currencyCodeIso4217: 'USD', requestFrom: `${m.sender.split('@')[0]}@s.whatsapp.net`, expiryTimestamp: 8600, amount: 10000, background: thumb }}}}, {quoted: m})
  }
...
`
  ...
  contextInfo: {externalAdReply: {showAdAttribution: true, containsAutoReply: true}}
  }
...
`
  ...
  buttons: buttons,
  headerType: 4,
  contextInfo:{externalAdReply: {contactVcard: true, showAdAttribution: true, containsAutoReply: true, title: `Hai ${pushname}`, body: `${wib}`, previewType: "IMAGE", thumbnailUrl: ``, thumbnail: global.thumb2, sourceUrl: global.roflix2}}
  }
  xcaa.sendMessage(m.chat, buttonMessage, { quoted: ftroli })
  } catch (err) {
  m.reply(mess.error)
...
case 'tiktok': case 'tiktoknowm': case 'ttnowm': case 'tiktokvideo': case 'ttvideo': {
  ...
  let buttons = [
     {buttonId: `tiktokaudio ${musim_duren_v}`, buttonText: {displayText: `Converter Audio`}, type: 1}
  ]
  let buttonMessage = {
...
case 'tiktokaudio': case 'tiktokmusic': case 'ttaudio': case 'ttkaudio': case 'ttmusik': {
  ...
  let buttons = [
      {buttonId: `tiktoknowm ${musim_duren_a}`, buttonText: {displayText: `Converter Video`}, type: 1}
  ]
  let buttonMessage = {
  document: {url: musim_duren_a},
  fileName: global.packname,
  mimetype: 'audio/mpeg',
  fileLength: 99999999,
  caption: global.mess.sukses,
