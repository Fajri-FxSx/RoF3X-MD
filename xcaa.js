...
// Fake Reply Sticker
  ...
  const rdmtrol = rdmfr[Math.floor(Math.random() * rdmfr.length)]
...
//Random Name Packname
  ...
  const rdmpck = packk[Math.floor(Math.random() * packk.length)]

//Random Name Author
  let utt = [global.author, global.author2]
  const rdmaut = utt[Math.floor(Math.random() * utt.length)]
...
//Random File
  ...
  let rdmm = [ggbh, ggbh2]
  const rdmmenu = rdmm[Math.floor(Math.random() * rdmm.length)]
...
`
  ...
  buttons: buttons,
  headerType: 4,
  contextInfo: {externalAdReply: {showAdAttribution: true, containsAutoReply: true, title: `Hai ${pushname}`, body: `${wib}`, mediaType: "IMAGE", thumbnailUrl: global.thumb2, mediaUrl: global.thumb2, thumbnail: global.thumb2, sourceType: global.linkgc, sourceId: global.linkgc, sourceUrl: global.linkgc}}
  }
  xcaa.sendMessage(m.chat, buttonMessage, { quoted: rdmtrol })
  } catch (err) {
  reply2(from)
...
case 'react': case 'reaction': {
  ...
  break
case 'sendsessions': case 'sendsession': case 'sendsessi': case 'sendsesi': {
  if (!isOwner && !m.key.fromMe) return m.reply(mess.ownerBot)
  try {
...
case 'linkgc': case 'linkgrup': case 'linkgroup': case 'linkgroups': {
  ...
  xcaa.sendMessage(m.chat, {text: `Link Grup : https://chat.whatsapp.com/${response}`, contextInfo: {mimetype: 'image/jpeg', text: `${rdmpck}`, externalAdReply: {showAdAttribution: true, title: `${groupMetadata.subject}`, mediaType: 2, body: `${wib}`, thumbnail: ppgcnya, previewType: "IMAGE", sourceUrl: `https://chat.whatsapp.com/${response}`}}}, { detectLink: true, quoted: m })
  } catch (err) {
  reply2(from)
  }
  }
...
case 'linkgc2': case 'linkgrup2': {
  ...
  let response = await xcaa.groupInviteCode(m.chat)
  let butlink = [
  {actionLink: `Salin`, buttonTitle: {url: `https://chat.whatsapp.com/${response}`}, type: 1}
  ]
...
case 'play': case 'ytplay': {
  ...
  buttons: buttons
  headerType: 2,
  contextInfo: {externalAdReply: {showAdAttribution: true, containsAutoReply: true, title: `${hasil.title}`, mediaType: "VIDEO", thumbnail: `${hasil.thumbnail}`, mediaUrl: `${hasil.url}`, sourceUrl: `${hasil.url}`}}
  }
  xcaa.sendMessage(m.chat, buttonMessage, { quoted: rdmtrol })
  } catch (err) {
  reply2(from)
