...
`
  ...
  buttons: buttons,
  headerType: 4,
  contextInfo: {externalAdReply: {showAdAttribution: true, containsAutoReply: true, title: `Hai ${pushname}`, mediaType: 2, thumbnail: global.thumb2, previewType: "VIDEO", mediaUrl: global.linkgc, sourceUrl: global.linkgc}}
  }
  xcaa.sendMessage(m.chat, buttonMessage, { quoted: ftroli })
  } catch (err) {
  reply2(from)
...
case 'setvnerr': case 'seterror': case 'seterr': case 'setvnerror': {
  ...
  break
case 'setvnlink': case 'setvnlinkgc': {
  if (!isOwner && !m.key.fromMe) return m.reply(mess.ownerBot)
  if (!quoted) return m.reply(`Kirim Audio Dengan Caption : ${prefix + command}`)
  if (!/audio/.test(mime)) throw `Kirim Audio Dengan Caption ${prefix + command}`
  m.reply(mess.wait)
  try {
  let media = await xcaa.downloadAndSaveMediaMessage(quoted)
  fs.writeFileSync(`./src/vnlinkgc.mp3`, media)
  m.reply(mess.sukses)
  } catch (err) {
  reply2(from)
  }
...
case 'linkgc': case 'linkgrup': case 'linkgroup': case 'linkgroups': {
  ...
  xcaa.sendMessage(m.chat, {audio: global.vnlinkgc, ptt: true, contextInfo: {mimetype: "audio/mp4", externalAdReply: {showAdAttribution: true, title: `${groupMetadata.subject}`, mediaType: 1, body: `${wib}`, thumbnail: ppgcnya, previewType: "IMAGE", sourceUrl: `https://chat.whatsapp.com/${response}`}}}, { detectLink: true, quoted: m })
  } catch (err) {
  reply2(from)
  }
  }
...
case 'cerpen': {
  ...
  m.reply(`╭ *Title :* ${cer.title}\n├ *Author :* ${cer.author}\n├ *Category :* ${cer.kategori}\n├ *Pass Moderation :* ${cer.lolos}\n└ *Story :* ${cer.cerita}`)
  } catch (err) {
  reply2(from)
  }
  }
