...
const { smsg, getGroupAdmins, formatp, tanggal, formatDate, getTime, isUrl, await, sleep, clockString, msToDate, sort, toNumber, enumGetKey, runtime, fetchJson, getBuffer, jsonformat, delay, format, logic, generateProfilePicture, parseMention, getRandom, pickRandom, createSerial } = require('./lib/myfunc')
...
//Random File
  ...
  const ggbh3 = (teks) => {
  let message = await prepareWAMessageMedia({ image: global.thumb2 }, { upload: xcaa.waUploadToServer })
...
( Q.S ${res.result.data.surah.name.transliteration.id} : ${res.result.data.number.inSurah} )`
  au = await xcaa.sendMessage(m.chat, {audio: { url: res.result.data.audio.primary }, mimetype: 'audio/mpeg'}, { quoted : m })
  xcaa.sendMessage(m.chat {text: `${txt}`}, {quoted: au})
  } catch (err) {
  reply2(from)
  }
  }
